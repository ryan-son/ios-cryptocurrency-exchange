...
- 메인 화면을 통해 전체 코인 목록을 확인 할 수 있습니다.
- 검색창을 이용해 원하는 코인을 검색 할 수 있습니다.
- 정렬 버튼을 이용하여, 가격순, 변동률순, 관심으로 정렬 할 수 있습니다.
...
### 개선할 수 있는 점
- 현재 현재값을 받는 `WebSocket`으로 부터 데이터를 전달받을 때 마다 현재 가격보다 높은 구매건, 현재 가격보다 낮은 판매건을 제거하는 작업을 하여 체결된 주문을 처리하고 있습니다. 화면을 새로 그리기 전에 체결한 정보를 받아 체결된 주문을 처리가 필요합니다.
